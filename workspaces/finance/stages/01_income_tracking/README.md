...
> **Status:** this stage's run-process now lives in the **admin dashboard** (`/finances` shows live
...
- **Upstream / reads from:** **Stripe** (read-only, via the dashboard `/finances` or `scripts/stripe-*.sh`); `biz.clients` (client identity).
