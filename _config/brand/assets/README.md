...
## Notes
The spec exists in [`visual/`](../visual/) (tokens, type, logo) and the canonical logo + brand assets now ship in [`packages/ui/assets/`](../../../packages/ui/assets/) — the JN monogram marks (`logo/mark-monogram.svg`, `mark-monogram-solid.svg`), the social card, and the email signature. This folder holds any additional **exported** variants (PNG/favicons/platform-sized social) produced from those sources. Assets are downstream of the spec: never edit a token by editing an exported file. If [`visual/`](../visual/) changes, re-export every affected asset here so the website and documents stay in sync. Name files by variant and intent (e.g. `logo-horizontal-dark.svg`) so both Claude and a human can pick the right one without guessing.
