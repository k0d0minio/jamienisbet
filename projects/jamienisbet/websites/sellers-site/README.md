...
## Contents
- `app/` — App Router. Root `layout.tsx` is the shell (sets `<html lang>` + locale-aware metadata from the cookie); each route group supplies its own chrome. `(site)/` is the seller landing (`page.tsx` + `layout.tsx` with `site-header`/`site-footer`); `(pitch)/pitch/` is the customer pitch page (`page.tsx` + minimal `layout.tsx`). `actions/referral.ts` holds the seller-lead Server Action (validates and replies in the active locale); `robots.ts`, `sitemap.ts`, `icon.svg`, `not-found.tsx` live at the app root.
