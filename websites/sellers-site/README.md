...
- The **marks** come from that same source. The pitch bar renders `LogoMark` and the seller header/footer inherit `LogoMark`/`LogoMarkSolid` through `@jamie-nisbet/app-shell`, so this app holds no artwork of its own — and the mark is untinted everywhere, because it is monochrome paper/ink and slate never appears inside it. The one place that needs a *file*, the favicon [`app/icon.png`](app/icon.png), carries the same 64px crop of the icon form's dark reading the other sites ship, cut from the brand artwork rather than redrawn; see [BRAND.md](../../packages/ui/BRAND.md) § Iconography → Logo.
