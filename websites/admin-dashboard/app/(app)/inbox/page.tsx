...
import { waitingOnYou } from "@/lib/inbox"
import {
  daysWaiting,
  isProspect,
  prospectLabel,
...
// No `dynamic = "force-dynamic"` here on purpose. `connection()` already
...
// The Inbox — home until Work took that place (D-6); moved here as it was, and
// rebuilt as a fast queue by later work in the admin-cockpit-redesign scope.
//
// The dashboard used to open on the roster — everyone, longest-waiting
...
async function loadDb(): Promise<DbReads> {
  ...
  try {
    ...
    // Open leads gone quiet, longest first, minus the ones already on the
    // queue below — the rule lives in lib/inbox.ts, where the rail's badge
    // reads it too, so the badge and this section count the same people.
    const waiting = waitingOnYou(clients, now)
...
// use for the repo sections Work folds. `listStrip()` shares its GitHub
...
export default async function InboxPage() {
  // The feed is live state — a read per request, never prerendered. It used
  // to get that from awaiting searchParams, back when it redirected the old
  // `/?filter=` leads bookmarks; those redirects stayed at `/` with Work.
  await connection()
...
// deep-linked to its own ticket on Work.
      ...
      {shown.map((ticket) => (
        ...
        >
          {/* Straight to the ticket on the board — the board is where a
              ...
              human sends. */}
          <Link
            href={`/?${new URLSearchParams({ t: `${ticket.repo.slug}/${ticket.id}` })}`}
          />
        </GroupedRow>
      ))}
    </GroupedSection>
