...
- The **marks** come from that same source. The header, footer, hero, 404 and Open Graph card render `LogoFull` — the full lockup, traced from the artwork — and the questionnaire's bar renders `LogoMark`, all from the package, so they follow it without this app holding any artwork. The one place that needs a raster is the favicon [`app/icon.png`](app/icon.png), the icon form's dark reading at 64px, cut from the brand artwork rather than redrawn. Nothing here draws the logo by hand; see [BRAND.md](../../packages/ui/BRAND.md) § Iconography → Logo.
