...
- D-26 — The desk tier had no menu, tabs or key–value list, so Build added `DeskMenu`, `DeskTabs` and `RecordSection`/`RecordRow`/`RecordBlock`/`RecordDisclosure` to `packages/ui` (the design-dna rule: a primitive another screen could use belongs in the package). This widens `touches:` beyond the spec's list — a spec gap, Build.
- D-27 — Delete from the profile now redirects to `/leads`, as the spec says; the code it replaced sent it to `/inbox`, which the spec misread as "keeps". Build.
- D-28 — Sheet interiors and the Deal card's inline term editors keep their app-tier form fields (`AppField`, `AppInput`, `AppSelect`): the desk tier has no field primitives yet, and the spec restyles only "as far as they render on this page". `retire-app-tier` owns the rest. Build.
