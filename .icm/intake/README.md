...
> row: `ready` → `today` → `in-progress` → `blocked` (missing = `ready`). Finished tickets
> are `git mv`'d to `_done/` — the folder move is the state change. The admin dashboard's
> Tickets board reads this folder from `main`.
