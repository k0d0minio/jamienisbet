...
- cut: 2026-09-23, from icm-board's `ticket-base-branch` epic (decision D37 there) — the
...
shows as open for the whole batch. Under D37 every client repo's ticket state lives on its
...
1. dashboard-reads-ticket-base — the board reads each repo's ticket base branch; the maintenance prompts follow D37 — depends-on: none
