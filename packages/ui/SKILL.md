...
- `assets/logo/` — the typographic JN mark: `logo-mark.png` (the letters as an alpha mask, inlined into `src/components/brand/logo-letters.ts`), `logo-full.svg` (tile + diamond + wordmark). Render it through `LogoMark`/`LogoMarkSolid`, never by redrawing the letters. `assets/brand/` — social card + email signature. `assets/lib/icons.js` — Lucide `<Icon>` helper for UMD/static surfaces.
...
- **Visuals:** one slate-blue (`--blue-600 #3A5A78`) as the interaction tint, cool-grey neutrals, logo in pure paper/ink (`--logo-tile` / `--logo-ink`, never slate), hairline borders, generous whitespace, blueprint-grid texture only, soft/low shadows, quick no-bounce motion. (Full rules in `BRAND.md` → Visual foundations.)
