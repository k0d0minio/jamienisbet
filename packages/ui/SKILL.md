...
- `assets/logo/` — the geometric JN mark: `logo-mark.svg` (currentColor), `logo-mark-solid.svg` (frame + mark, baked dark), `logo-full.svg` (tile + diamond + wordmark). `assets/brand/` — social card + email signature. `assets/lib/icons.js` — Lucide `<Icon>` helper for UMD/static surfaces.
